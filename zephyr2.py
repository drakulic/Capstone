...
model = smf.ols(formula='sale_price ~ district + bedroom + bath + parking +\
 sqft + sold + home_own_ass + day_on_market + list_price + single_f_h + \
 condo +
  dist_no', data=df)
linreg = model.fit()
...
'''
print '\nmodel without date: \n'
model_nodate = smf.ols(formula='sale_price ~ district + bedroom + bath + \
parking + sqft + home_own_ass + day_on_market + list_price + single_f_h\
 + condo
 + dist_no', data=df)
...
model_y_m = smf.ols(formula='sale_price ~ district + bedroom + bath + parking +\
 sqft + home_own_ass + day_on_market + list_price + single_f_h + condo\
  + dist_no + sold_year + sold_month', data=df)
...
model_y_m = smf.ols(formula='sale_price ~ district + bedroom + bath + \
parking +
 sqft + home_own_ass + day_on_market + list_price + single_f_h + condo\
  + dist_no + sold_year + sold_month', data=df_sold)
...
# X_train, X_test, y_train, y_test = train_test_split(X_sold, y_sold,
...
model_sold_list = smf.ols(formula='np.log10(sale_price) ~ district + bedroom + \
bath + parking + sqft + home_own_ass + day_on_market + np.log10(list_price)\
 + single_f_h + condo + dist_no + sold_year + sold_month', data=df_sold)
linreg3 = model_sold_list.fit()
print '\nmodel with log10 list price: ', linreg3.summary()

model_sold_no_list = smf.ols(formula='np.log10(sale_price) ~ district + \
bedroom + bath + parking + sqft + home_own_ass + day_on_market + single_f_h +\
 condo + dist_no + sold_year + sold_month', data=df_sold)
...
for i in xrange(30):
    print df_sold.index[4000+i].date(), '    ', df_sold.sale_price[4000+i], '    ',
    np.around(np.power(10, linreg3.fittedvalues[4000+i]), decimals=2),
    '     ', np.around(np.power(10, linreg4.fittedvalues[4000+i]), decimals=2), '    ',
    np.around(np.power(10, linreg3.fittedvalues[4000+i]) -
    np.power(10, linreg4.fittedvalues[4000+i]), decimals=2)
...
ax.plot([y_train.min(), y_train.max()], [y_train.min(),
        y_train.max()], 'k--', lw=4)
...
# Explained variance score: 1 is perfect prediction
print
print('lr3 no list price Train Variance score: %.2f' %
...
ax.plot([y_sold.min(), y_sold.max()], [y_sold.min(), y_sold.max()],
        'k--', lw=4)
...
print 'Date        Square-Foot     Sale-price    Predicted price w LP   Predicted w/o LP    S-P error'
for i in xrange(30):
    print X_test.index[i].date(), '     ', X_test.sqft[i], '     ', y_test[i],
    '     ', np.round(predicted_test2[i], 2), '     ',
    np.round(predicted_all[i], 2), '    ', np.round(y_test[i]-predicted_all[i], 2)
#######################################
...
ax.plot([y_train_log.min(), y_train_log.max()],
        [y_train_log.min(), y_train_log.max()], 'k--', lw=4)
...
ax.plot([y_test_log.min(), y_test_log.max()], [y_test_log.min(),
        y_test_log.max()], 'k--', lw=4)
...
for i in xrange(30):
    print X_test.index[i].date(), '     ', X_test.sqft[i], '     ',
    np.power(10, y_test_log[i]), '     ', np.power(10, predicted_test_log[i]).round(2)
...
fig, ax = plt.subplots(figsize=(25, 15))
fig.suptitle('whole set log10 price LinearRegression without list price',
             fontsize=20)
ax.scatter(y_sold_log, predicted_all_log)
ax.plot([y_sold_log.min(), y_sold_log.max()],
        [y_sold_log.min(), y_sold_log.max()], 'k--', lw=4)
ax.set_xlabel('Measured', fontsize=20)
ax.set_ylabel('Predicted', fontsize=20)
plt.show()
...
print
print('lr_log_all All Variance score: %.2f' %
      lr_log_all.score(X_sold, y_sold_log))

print
print 'Date        Square-Foot     Sale-price    Predicted price '
for i in xrange(30):
    print X_test.index[i].date(), '     ', X_test.sqft[i], '     ',
    np.power(10, y_test_log[i]), '     ',
    np.power(10, predicted_all_log[4000+i]).round(2)
print
print
###############################################
'''
...
def stage_score_plot(estimator, train_x, train_y, test_x, test_y):
    ...
    plt.plot(train_scores, alpha=.5, label='%s Train - Rate %s' %
             (name, learn_rate))
    plt.plot(test_scores, alpha=.5, label='%s Test - Rate %s' %
             (name, learn_rate), ls='--')
...
#################
# The best model
gdbr_best = GradientBoostingRegressor(alpha=0.9,
...
print 'Date        Square-Foot     Sale-price    Predicted price '
for i in xrange(30):
    print test_x.index[i].date(), '     ', test_x.sqft[i], '     ',
    np.power(10, test_y[i]), '     ', np.power(10, test_ypred_b[i]).round(2)
print
print
...
#################
sale_avg = df_sold.sale_price.mean()
...
# on test set
sale_avg = df_sold.sale_price[4000:].mean()
with_list_avg = np.around(np.power(10, linreg3.fittedvalues[4000:]), decimals=2).mean()
without_list_avg = np.around(np.power(10, linreg4.fittedvalues[4000:]), decimals=2).mean()
s_w = sale_avg - with_list_avg
s_wo = sale_avg - without_list_avg
gb_s_avg = np.power(10, test_y).mean()
gb_pred_avg = np.power(10, test_ypred_b).round(2).mean()
...
print
print 'test set sale price avg: ', sale_avg
print '\ntest set with list price avg: ', with_list_avg
print '\ndiff: ', s_w
print
print 'test set without list price avg: ', without_list_avg
print '\ndiff: ', s_wo
print
print 'sale price avg for GB test', gb_s_avg
print '\nGB predicted avg: ', gb_pred_avg
